...
export function FirebaseForgotOtpPage() {
  ...
  useEffect(() => {
    if (!rawPhone) navigate('/forgot-password', { replace: true })
  }, [rawPhone, navigate])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.reset_token) {
        ...
        pushToast({ title: res.detail || 'Xato yuz berdi', variant: 'error' })
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <div>
          <p className="text-xs text-white/40 font-medium">Parolni tiklash — SMS yuborildi</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="000000"
            autoFocus
            className="h-14 w-full rounded-2xl text-center text-3xl font-black outline-none transition-all"
            style={{
      ...
      >
        <ArrowLeft className="h-3.5 w-3.5" /> Ortga qaytish
      </button>
    </AuthFormShell>
  )
}
