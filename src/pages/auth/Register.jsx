    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          style={{ background:'rgba(255,255,255,0.07)', border:`1.5px solid ${error?'rgba(248,113,113,0.5)':'rgba(255,255,255,0.1)'}`, color:'#f0f6ff', fontFamily:'inherit', paddingRight: isPwd ? 44 : 16 }}
...
export function Register({ pendingFarm = false, pendingDriver = false }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // 1. Backend register — agar raqam band bo'lsa 409 xatosi
      ...
      // 2. Telegram ulangan bo'lsa OTP yuborilgan, ulangan bo'lmasa bot linki
      if (result.linked) {
        pushToast({ title: 'Tasdiqlash kodi Telegram ga yuborildi! 📱', variant: 'success' })
      } else {
        pushToast({ title: "Telegram botni ulab, tasdiqlash kodini oling", variant: 'info' })
      }

      // 3. OTP sahifasiga o'tish
      ...
      if (err.message?.includes('allaqachon')) {
        pushToast({ title: "Bu raqam allaqachon ro'yxatdan o'tgan! Kirish sahifasiga o'ting.", variant: 'error' })
      } else {
  ...
  return (
    ...
    >
      <form className="space-y-3.5" onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <AuthField label="Telefon" icon={Phone} error={e.phone?.message} {...register('phone')} placeholder="+998 90 000 00 00" inputMode="tel" />
        ...
        <button type="submit" disabled={loading}
          className="flex h-11 w-full items-center justify-center gap-2.5 rounded-xl text-[14px] font-semibold text-white disabled:opacity-50 mt-1"
